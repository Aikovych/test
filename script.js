...
/* Plain modal */
createModal({ modal: 'plainModal', track: 'plainTrack', prev: 'plainPrev', next: 'plainNext', dots: 'plainDots', open: 'openPlain', close: 'closePlain' });

/* ============================================================
   HERO 3D PARALLAX — mouse/gyro
   ============================================================ */
(function initHeroParallax() {
  ...
  const depths = [0.04, 0.025, 0.018, 0.032]; // parallax depth per card

  function applyParallax(x, y) {
    // x, y: -1 to 1 relative to center
    cards.forEach((card, i) => {
      const d = depths[i] || 0.03;
      const moveX = x * 60 * d * 100;
      const moveY = y * 60 * d * 100;
      card.style.transition = 'transform 0.6s cubic-bezier(0.25,0.46,0.45,0.94)';
      const base = card.style.animationPlayState;
      card.style.transform = `rotateY(${parseFloat(card.dataset.ry || 0) + x * 8}deg) rotateX(${parseFloat(card.dataset.rx || 0) - y * 6}deg) translateZ(${card.dataset.tz || 30}px) translate(${moveX}px, ${moveY}px)`;
    });
    glows.forEach((g, i) => {
      g.style.transform = `translate(${x * (i + 1) * 20}px, ${y * (i + 1) * 15}px) scale(${1 + Math.abs(x) * 0.1})`;
    });
  }
  ...
  if (window.DeviceOrientationEvent) {
    window.addEventListener('deviceorientation', (e) => {
      const x = Math.min(Math.max((e.gamma || 0) / 30, -1), 1);
      const y = Math.min(Math.max(((e.beta || 0) - 30) / 30, -1), 1);
      applyParallax(x, y);
    });
  }
})();
